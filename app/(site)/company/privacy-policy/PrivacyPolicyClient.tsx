...
import type { PrivacyPolicy } from '@/sanity/sanity-utils';
...
const portableTextComponents: PortableTextComponents = {
  block: {
    ...
    h2: ({ children }) => <h2 className="text-xl font-bold text-white mt-6 mb-4">{children}</h2>,
    h3: ({ children }) => <h3 className="text-lg font-bold text-white mt-4 mb-3">{children}</h3>,
    h4: ({ children }) => <h4 className="text-base font-bold text-white mt-3 mb-2">{children}</h4>,
    blockquote: ({ children }) => (
      <blockquote className="border-l-4 border-[#0062d1] pl-4 my-4 italic text-slate-300">
        {children}
      </blockquote>
    ),
  },
    ...
    link: ({ value, children }) => {
      ...
      return (
        <a 
          ...
          className="text-[#0062d1] hover:underline"
        >
          {children}
        </a>
      );
  ...
  list: {
    bullet: ({ children }) => (
      <ul className="list-disc list-inside space-y-2 mb-4 marker:text-[#0062d1]">{children}</ul>
    ),
    number: ({ children }) => (
      <ol className="list-decimal list-inside space-y-2 mb-4 marker:text-[#0062d1]">{children}</ol>
    ),
  },
  listItem: {
...
export default function PrivacyPolicyClient({ policy, formattedDate }: PrivacyPolicyClientProps) {
  ...
  // Split explicit data list into two columns
  const explicitDataList = policy.explicitDataList || [
    'Full Name & Job Title',
  ...
  const midPoint = Math.ceil(explicitDataList.length / 2);
  const explicitDataCol1 = explicitDataList.slice(0, midPoint);
  const explicitDataCol2 = explicitDataList.slice(midPoint);
      ...
      <section className="relative pt-32 pb-20 px-6 border-b border-white/5 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 leading-tight animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
            {policy.title || 'Global Privacy Policy'}
          </h1>
          ...
          <p className="text-lg md:text-xl text-slate-400 max-w-3xl font-light leading-relaxed mb-8 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            {policy.heroDescription || 'Our commitment to your data sovereignty and privacy, compliant with GDPR, CCPA, and international standards.'}
          </p>
              ...
              <div className="mt-8 pt-6 border-t border-white/5">
                <div className="flex items-center gap-3 opacity-60">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-slate-400 font-bold">{policy.complianceBadge || 'GDPR & CCPA Ready'}</div>
                    </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-9 space-y-20">
            ...
            <section id="introduction" className="scroll-mt-32">
              ...
              <div className="prose prose-invert prose-lg max-w-none text-slate-400">
                {policy.introductionBody ? (
                  <PortableText value={policy.introductionBody} components={portableTextComponents} />
                ) : (
                  <p>
                ...
                <div className="bg-[#0B0F19] border-l-4 border-[#0062d1] p-6 my-6 rounded-r-xl">
                  <p className="text-sm text-white m-0 font-medium">
                    {policy.transparencyStatement || 'Transparency Statement: We operate with full transparency regarding data collection protocols. This policy details the technical and legal frameworks governing your information.'}
                  </p>
                </div>
              </div>
            </section>
            ...
            <section id="sovereignty" className="scroll-mt-32">
              ...
              <p className="text-slate-400 mb-6">
                {policy.sovereigntyIntro || 'Your data is processed under strict legal frameworks depending on your region. We maintain distinct legal entities to ensure compliance with local regulations.'}
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-[#0B0F19] p-6 rounded-xl border border-white/10 relative overflow-hidden group">
                    ...
                    <div className="flex items-center gap-3 mb-4 text-white relative z-10">
                        ...
                        <h3 className="font-bold">{policy.usaJurisdiction?.title || 'USA Jurisdiction'}</h3>
                    </div>
                    <p className="text-xs text-slate-500 mb-2 font-mono uppercase tracking-wider">
                      Governing Law: {policy.usaJurisdiction?.governingLaw || 'Delaware State Law'}
                    </p>
                    <p className="text-sm text-slate-400">
                        {policy.usaJurisdiction?.description || 'For clients in North America, contracting and data processing are handled by our US entity in Wilmington, DE. Compliant with CCPA.'}
                    </p>
                </div>
                <div className="bg-[#0B0F19] p-6 rounded-xl border border-white/10 relative overflow-hidden group">
                    ...
                    <div className="flex items-center gap-3 mb-4 text-white relative z-10">
                        ...
                        <h3 className="font-bold">{policy.euJurisdiction?.title || 'EU Jurisdiction'}</h3>
                    </div>
                    <p className="text-xs text-slate-500 mb-2 font-mono uppercase tracking-wider">
                      Governing Law: {policy.euJurisdiction?.governingLaw || 'Estonian & EU Law'}
                    </p>
                    <p className="text-sm text-slate-400">
                        {policy.euJurisdiction?.description || 'For global and EU clients, operations are governed by our entity in Tallinn, Estonia. Fully GDPR compliant regarding data portability and privacy.'}
                    </p>
                </div>
              </div>
            </section>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-[#0B0F19] p-6 rounded-xl border border-white/5">
                    <div className="flex items-center gap-3 mb-4 text-white border-b border-white/5 pb-3">
                        ...
                        <h3 className="font-bold text-lg">{policy.explicitDataTitle || 'Explicit Data (Input)'}</h3>
                    </div>
                    <p className="text-slate-400 text-sm mb-4">
                      {policy.explicitDataDescription || 'Data you voluntarily provide through our Inquiry Forms, Project Simulator, or direct correspondence.'}
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <ul className="list-disc list-inside text-sm text-slate-400 space-y-2 marker:text-[#0062d1]">
                            {explicitDataCol1.map((item, idx) => (
                              <li key={idx}>{item}</li>
                            ))}
                        </ul>
                        <ul className="list-disc list-inside text-sm text-slate-400 space-y-2 marker:text-[#0062d1]">
                            {explicitDataCol2.map((item, idx) => (
                              <li key={idx}>{item}</li>
                            ))}
                        </ul>
                    </div>
                ...
                <div className="bg-[#0B0F19] p-6 rounded-xl border border-white/5">
                    <div className="flex items-center gap-3 mb-4 text-white border-b border-white/5 pb-3">
                        ...
                        <h3 className="font-bold text-lg">{policy.implicitDataTitle || 'Implicit Data (Telemetry)'}</h3>
                    </div>
                    <p className="text-slate-400 text-sm mb-4">
                      {policy.implicitDataDescription || 'Technical data automatically collected to optimize system performance and security.'}
                    </p>
                    <ul className="list-disc list-inside text-sm text-slate-400 space-y-2 marker:text-amber-500">
                        {(policy.implicitDataList || [
            ...
            <section id="data-usage" className="scroll-mt-32">
              ...
              <div className="prose prose-invert max-w-none text-slate-400 mb-6">
                {policy.dataUsageBody ? (
                  <PortableText value={policy.dataUsageBody} components={portableTextComponents} />
                ) : (
                  <p>
              ...
              <div className="bg-emerald-900/10 border border-emerald-500/20 rounded-lg p-4 flex gap-4">
                ...
                <div>
                    <h4 className="text-emerald-400 font-bold text-sm mb-1">
                      {policy.gdprGuaranteeTitle || 'GDPR Compliance Guarantee'}
                    </h4>
                    <p className="text-xs text-emerald-100/70 leading-relaxed">
                        {policy.gdprGuaranteeText || 'Any transfer of data outside the EEA is protected by Standard Contractual Clauses (SCCs) approved by the European Commission, ensuring your data rights travel with your data.'}
                    </p>
                </div>
              </div>
            </section>
            ...
            <section id="security" className="scroll-mt-32">
              ...
              <div className="bg-[#0B0F19] border border-white/10 rounded-xl p-8 text-center">
                ...
                <h3 className="text-xl font-bold text-white mb-4">
                  {policy.securityTitle || 'Enterprise-Grade Encryption'}
                </h3>
                <p className="text-slate-400 text-sm max-w-2xl mx-auto leading-relaxed mb-6">
                    {policy.securityDescription || 'We adhere to a "Security by Design" philosophy. All sensitive project data is treated as high-value intellectual property.'}
                </p>
                <div className="flex flex-wrap justify-center gap-4 text-xs font-mono text-slate-500">
                    {(policy.securityFeatures || [
                      'AES-256 Data at Rest',
                      'TLS 1.3 Data in Transit',
                      'MFA Access Control',
                    ]).map((feature, idx) => (
                      <span key={idx} className="px-3 py-1 bg-white/5 rounded border border-white/10">{feature}</span>
                    ))}
                </div>
              </div>
            </section>
            ...
            <section id="gdpr-rights" className="scroll-mt-32">
              ...
              <p className="text-slate-400 mb-6">
                {policy.gdprRightsIntro || 'We empower you with full control over your digital footprint. You have the right to:'}
              </p>
                ...
                {(policy.gdprRights || [
                  ...
                  <div key={idx} className="group flex items-start gap-4 p-4 rounded-lg hover:bg-white/5 transition-colors border border-transparent hover:border-white/5">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 text-xs mt-1">{right.description}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section id="cookies" className="scroll-mt-32">
              ...
              <p className="text-slate-400 mb-4">
                {policy.cookiesIntro || 'We utilize minimal cookies strictly necessary for system operation.'}
              </p>
              <ul className="list-disc list-inside text-slate-400 space-y-2 ml-4 mb-6">
                {(policy.cookiesList || [
                  ...
                  <li key={idx}><strong>{cookie.name}:</strong> {cookie.description}</li>
                ))}
              </ul>
            </section>
            ...
            <section id="contact" className="scroll-mt-32 pb-12">
              <div className="bg-gradient-to-r from-[#0062d1]/20 to-transparent border border-[#0062d1]/30 rounded-2xl p-8">
                <h2 className="text-2xl font-bold text-white mb-4">
                  {policy.contactTitle || 'Contact Data Protection Officer'}
                </h2>
                <p className="text-slate-300 mb-6">
                  {policy.contactDescription || 'To exercise your rights or report a vulnerability, initiate a secure channel with our DPO.'}
                </p>
                <div className="flex flex-col sm:flex-row gap-4">
                  <a 
                    href={`mailto:${policy.dpoEmail || 'privacy@gimmir.com'}`} 
                    className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-[#0062d1] hover:bg-[#0052b3] text-white font-bold rounded-lg transition-colors"
                  >
                    <Server size={18} />
                    <span>{policy.dpoEmail || 'privacy@gimmir.com'}</span>
                  </a>
